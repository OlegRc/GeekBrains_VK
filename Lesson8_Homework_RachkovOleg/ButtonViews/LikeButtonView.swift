...
class LikeButtonView: UIView {
    ...
    let likedImageOff: UIImageView = {
        ...
        image.image = UIImage.init(named: "like_off")
    ...
    let likedImageOn: UIImageView = {
        let image = UIImageView()
        image.backgroundColor = UIColor.white.withAlphaComponent(0)
        image.image = UIImage.init(named: "like_on")
        image.translatesAutoresizingMaskIntoConstraints = false
        return image
    }()
    ...
    override init(frame: CGRect) {
        ...
        tapGestureRecognizer.addTarget(self, action: #selector(likeButtonPressed))
        ...
        likesCountLabelView.centerXAnchor.constraint(equalTo: likesContainer.centerXAnchor).isActive = true
    ...
    func setLikeButton() {
        ...
        if isLiked {
            likesCountLabelView.textColor = .white
            likedImageOff.isHidden = true
            likedImageOn.isHidden = false
        } else {
            likesCountLabelView.textColor = .black
